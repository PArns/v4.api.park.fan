...
- [Ride downtime](docs/analytics/ride-downtime.md) - **PLAN, nothing built (2026-09-06)**: what a downtime detector can and cannot measure. An outage is an INTERVAL, never a day. `DOWN` comes from themeparks-wiki alone (0.3-0.7 % of live rows, a minority of parks) and `ConflictResolverService` rewrites it to OPERATING whenever a second source reports >= 5 min, so the numerator is partly a measure of which feeds a park has. The denominator is the ride's OWN operating minutes, which is why Wakobato's Sunday closure needs no rule at all. Count and duration are publishable behind an event floor; a probability for the next outage is refused, with the sample-size arithmetic written down. Also: `downCount` counts DOWN-HOURS including hours outside opening times (Revenge of the Mummy, 12 Aug: 16 on a 10-hour day), so it is not an event count.
