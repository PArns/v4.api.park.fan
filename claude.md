...
- [Ride downtime](docs/analytics/ride-downtime.md) - **MEASURED AND PARTLY BUILT (2026-09-06)**: phase 0 run against production — 150 132 outage events over 90 days, 2228 rides, 78 parks. An outage is an INTERVAL, never a day, and its denominator is the ride's OWN operating minutes (so Wakobato's Sunday closure needs no rule). **Two gates were wrong and are fixed**: the artefact test measured outage length rather than feed quality (r = 0.996 with "outages under an hour" — `queue_data` is a change log, so a short outage writes exactly one row) and marked 47 of 78 parks unreadable; it now measures timestamp resolution and catches nobody. Censoring was reported as `thin_events`, its opposite. The event floor of 24 is confirmed (1324 rides clear it). **§6a: how much longer a running outage lasts** is published — NOT the refused forecast, because the event is observed rather than predicted, and counting in operating minutes takes censoring from 68 % to 15.6 %; out-of-sample error 2.55 pp, hazard falls 54 %→8.5 % so no exponential. Also: `downCount` counts DOWN-HOURS outside opening times (Revenge of the Mummy: 16 on a 10-hour day), so it is not an event count.
...
- [What we can forecast, and how far](docs/ml/long-range-forecasting.md) - **2026-09-03**: measured horizons per model (CatBoost daily ~183 d, TFT 60 d, shape 14 d, PCN 1 d), TFT's lead-time error curve (MAE 11.66 → 15.09 from 1 to 60 days), and two measurements that overturned their own premise: a per-weekday intraday shape is WORSE than the weekday-blind one (noise floor 0.120 vs a 0.15 "signal"), and the shape model is at parity rather than 35% behind — its shadow board compares a 14-day forecast against CatBoost's always-≤24 h one. Also the conditioning-on-outcome trap that makes a well-calibrated model look 50% biased, and the evidence that TFT beats CatBoost by 28% on NON-headliners too. **2026-09-11 (§6–§8)**: the served grid is now the same **six** lead buckets the forward archive samples (`d1/d3/d7/d14/d30/d60`, 18 cells, all clearing the 500-comparison floor) — it was four, which answered a 10-day question with the 30-day cell. Each band widens by the same ~4 min over 60 days, but that is +19% busy vs **+45% quiet**, which is why the served figure is a looked-up cell and never a multiplier. And **a per-ride curve is measured as impossible**: of 2,643 rides none clears 100 comparisons in all six buckets, because `d1` spans one lead distance (≤1 comparison per ride per target day, max 45) — while `d30`/`d60` only clear 500 by pooling 16 and 30 distances, so feasibility runs opposite to usefulness. The forward archive is 7 days old: buckets 7–60 hold rows but no scores, and `d60` cannot report before 2026-11-02.
...
- [What an attraction is for (`attractionKind`)](docs/frontend/attraction-kind.md) - The hand-decided `RIDE` / `TRANSPORT` / `SHOW` / `WALKTHROUGH` on every attraction. Null means unjudged, never "it is a ride", and it can be derived from neither the name nor the upstream `attractionType` — of 151 transport-sounding attractions, 17 are headliners and 3 are actually transport.
- [Where you stay dry (`indoorOutdoor`)](docs/frontend/indoor-outdoor.md) - The hand-decided `indoor` / `outdoor` / `covered_queue` on every attraction, for the rain plan. Null means nobody checked and is neither; no feed carries it, the source of each value is on the curation's audit row.
...
- [Works periods (`worksPeriod`)](docs/frontend/works-period.md) - The hand-curated rebuild/refit window per ride, on the attraction payload and on every ride in the park payload: `{ from, to, toUncertain }`, park-local and both bounds inclusive. **Never render it as an outage** — the API reports no outage at all inside a running window, because the feed cannot tell a rebuild from a breakdown. `toUncertain` marks an end date we estimated rather than one the park published, and it is never true without a `to`. The block says nothing about today: compare against the park's `timezone`, never the reader's clock. Absent means nobody curated one, never that the ride is running.
- [One day, ride by ride (`/plan/day`)](docs/frontend/plan-day-endpoint.md) - The trip-planner series: per-ride hourly waits for one date plus that day's context. `tier` is derived from the curves that were built (never from the distance), a day inside the model's 24-hour window is part measured and part composed with `source` on the hours that differ, `dayPeak` is the same statistic on every tier, and past the operator's publishing horizon the open window falls back to measured hours (`hoursSource: "observed"`). **A day that crosses midnight** (`closeHour < openHour`, La Ronde every day) keeps the operator's wall-clock pair in the context while `hours[].hour` runs past 23 — 24 is that day's midnight; unfold with `unfoldedCloseHour`, never test `hour > closeHour`. Also **`opensAt`** (a ride opens later than its park — 16 rides at PHL, and the gap is SEASONAL so the lookup is keyed on the park's own opening), **`expectedError` + `accuracy.basis`** (measured error by predicted band × lead; `unmeasured` past 60 days is the do-not-plan flag), and **projected showtimes** (no feed publishes them ahead of today; a projection is dropped where the SHOW's own season says the planned month is out, while a `scheduled` time never is). **A ride that cannot open on the planned day is absent, not closed** — the detector's season, and now also the hand-written works window (`curated_out_of_service_from`/`_to`, both bounds inclusive, asked about the planned day); a live `OPERATING` row overrules a stale SEASON for today at a cost of 0 or 1 extra query, and overrules the curated window never.
- [Stored plans & web push (`/v1/trips`, `/v1/push`, `/v1/push/ride-alerts`, `/v1/push/show-follows`)](docs/frontend/trips-and-push.md) - The id IS the credential (96 random bits, no accounts), full-replace writes, the payload floor that keeps an unauthenticated write endpoint from being a file host, and the push contract: ask `GET /v1/push` before offering the switch, endpoints only at known push services, the language stored per subscriber. `tripId`/`topics` on `POST /v1/push/subscriptions` are optional and preserved-on-omit, never cleared, because the same row now also serves ride alerts (a per-attraction wait-time threshold, checked from `WaitTimesProcessor` each poll cycle via the pure `diffRideAlerts`, dedup by an `armed` column rather than Redis) and followed shows (a second branch of the existing five-minute push cron, via the pure `dueShowNotifications`).
