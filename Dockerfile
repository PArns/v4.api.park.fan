...
# Create scripts directory and generate-swagger-spec.ts if it doesn't exist
# (scripts/ is in .gitignore, so we create it here for Docker builds)
RUN mkdir -p scripts && \
    cat > scripts/generate-swagger-spec.ts << 'EOFSCRIPT'
/**
...
RUN apk add --no-cache curl && npm install -g pnpm@11.5.0
