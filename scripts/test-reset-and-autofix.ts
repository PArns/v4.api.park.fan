...
async function run() {
  ...
  `, [park[0].id]);
    ...
    if (mappings.length === 2 && attractions[0].with_land > 0) {
        ...
        console.log('   - Both park mappings created (themeparks-wiki + queue-times)');
        console.log('   - No conflicts detected (or automatically resolved)');
        console.log(`   - Land data populated for ${attractions[0].with_land}/${attractions[0].total} attractions`);
    } else {
