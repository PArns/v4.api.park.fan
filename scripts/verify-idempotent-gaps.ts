...
import { ScheduleEntry, ScheduleType } from '../src/parks/entities/schedule-entry.entity';
...
async function verifyIdempotentGapFilling() {
    ...
    const scheduleRepo = app.get<Repository<ScheduleEntry>>(getRepositoryToken(ScheduleEntry));
    ...
    console.log('--- Step 3: Running fillScheduleGaps (should do nothing yet) ---');
    await parksService.fillScheduleGaps(park.id);

    let entry = await scheduleRepo.findOne({ where: { parkId: park.id, date: testDate } });
    console.log('Entry after 1st gap fill:', entry ? { isHoliday: entry.isHoliday, holidayName: entry.holidayName } : 'NULL');
    ...
    console.log('--- Step 5: Running fillScheduleGaps AGAIN (should update existing entry) ---');
    await parksService.fillScheduleGaps(park.id);

    entry = await scheduleRepo.findOne({ where: { parkId: park.id, date: testDate } });
    console.log('Entry after 2nd gap fill:', entry ? { isHoliday: entry.isHoliday, holidayName: entry.holidayName } : 'NULL');

    if (entry && entry.isHoliday && entry.holidayName === 'Zweiter Weihnachtstag') {
