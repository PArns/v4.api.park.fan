...
async function run() {
  ...
  `, [fakeWrongParkId, queueTimesId]);
    ...
    console.log('  This will call ParkMetadataProcessor which will detect and resolve the conflict\n');
  ...
  `, [parkId, queueTimesId]);
    ...
    const qtMapping = finalMappings.find((m: any) =>
        m.external_source === 'queue-times' && m.external_entity_id === queueTimesId
    );

    if (qtMapping && qtMapping.internal_entity_id === parkId) {
        console.log('\n✅ SUCCESS! Conflict automatically resolved.');
        console.log(`   queue-times:${queueTimesId} now correctly points to Phantasialand`);
    } else {
