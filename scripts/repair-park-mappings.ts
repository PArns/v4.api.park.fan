...
async function run() {
  ...
  for (const park of parks) {
    if (park.queueTimesEntityId) {
      ...
      if (!existing) {
        console.log(
          `⚠️ Missing QT mapping for park ${park.name} (QT ID: ${park.queueTimesEntityId}). Creating...`,
        );
        ...
        try {
          ...
          console.error(
            `   ❌ Failed to create mapping for ${park.name} (likely duplicate):`,
            err.message,
          );
        }
      }
      ...
      if (park.wikiEntityId) {
        ...
        if (!existingWiki) {
          console.log(
            `⚠️ Missing Wiki mapping for park ${park.name} (Wiki ID: ${park.wikiEntityId}). Creating...`,
          );
          try {
            await mappingRepo.save({
              internalEntityId: park.id,
            ...
            console.error(
              `   ❌ Failed to create Wiki mapping for ${park.name}:`,
              err.message,
            );
          }
        }
