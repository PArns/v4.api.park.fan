...
def test_feature_engineering():
    ...
    df = pd.DataFrame(
        {
            ...
            "waitTime": [
                10 + i for i in range(len(timestamps))
    ...
    parks_metadata = pd.DataFrame(
        {
            ...
            "influencingRegions": [
                [
                    {"countryCode": "FR", "regionCode": None},
                    {"countryCode": "NL", "regionCode": None},
                ]
            ],
            "latitude": [50.0],
            "longitude": [10.0],
    ...
    df = features.add_holiday_features(
        df, parks_metadata, datetime(2025, 12, 1), datetime(2025, 12, 2)
    )
    ...
    if "hour_sin" in df.columns:
        ...
        if abs(row0["hour_sin"]) < 0.01 and abs(row0["hour_cos"] - 1.0) < 0.01:
            ...
            print(
                f"   ❌ Cyclical values INCORRECT for 00:00: sin={row0['hour_sin']}, cos={row0['hour_cos']}"
            )
    else:
    ...
    if row0["is_holiday_neighbor_1"] == 1:
        ...
        print("   ❌ Neighbor Public Holiday NOT detected")
