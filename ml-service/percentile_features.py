...
from db import fetch_queue_aggregates
...
def add_percentile_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'temperature_avg' in df.columns and 'parkId' in df.columns:
        ...
        for park_id in df['parkId'].unique():
            ...
            park_df['temp_p90_rolling'] = park_df['temperature_avg'].rolling(
    ...
    if 'windSpeedMax' in df.columns and 'parkId' in df.columns:
        ...
        for park_id in df['parkId'].unique():
            ...
            park_df['wind_p90_rolling'] = park_df['windSpeedMax'].rolling(
                window=14*24,
                min_periods=7*24
            ).quantile(0.90)
    ...
    print(f"   ✓ Percentile features added")
    return df
