...
def add_attraction_type_feature(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.warning(f"Failed to fetch attraction types: {e}. Using default 'UNKNOWN'.")
        df['attraction_type'] = 'UNKNOWN'
    
    return df


def add_park_attraction_count_feature(df: pd.DataFrame, parks_metadata: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    if 'attraction_count' in parks_metadata.columns:
        ...
        df['park_attraction_count'] = df['parkId'].map(lambda x: park_counts.get(str(x), 0)).fillna(0).astype(int)
        return df
