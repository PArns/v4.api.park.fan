...
none, and only when the winner holds none of them. The set is checked on the way
out as well, because these columns are reachable by hand and the value is about
to be copied onto a row that outlives the merge — a losing window that states
nothing on its own (a bare `to_uncertain`, an estimate flag with no end date)
stays behind rather than travelling. What arrives is a window somebody could
