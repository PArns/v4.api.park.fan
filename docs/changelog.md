...
- **ML training: same `>= 5` and schedule filters** (`ml-service/db.py`): Training data extraction used `waitTime >= 0` and had no schedule filter. Now applies `waitTime >= 5` and the same schedule JOIN (with `JOIN parks p` for timezone). Requires retraining to take effect.
- **ML training: `fetch_recent_wait_times` `>= 5` filter** (`ml-service/predict.py`): Inference recent-wait lookup also aligned to `waitTime >= 5` + schedule JOIN.
